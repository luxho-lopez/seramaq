...
if (isset($_GET['q'])) {
    $datos = array();
    $nombre = $_GET['q'];
    // $cliente = mysqli_query($conexion, "SELECT * FROM cliente WHERE contrato LIKE '%$contrato%' OR nombre LIKE '%$nombre%' OR planta LIKE '%$planta%' AND estado = 1");
    $cliente = mysqli_query($conexion, "SELECT * FROM cliente WHERE nombre LIKE '%" . $nombre . "%' OR contrato LIKE '%" . $nombre . "%' OR planta LIKE '%" . $nombre . "%' AND estado = 1");
    while ($row = mysqli_fetch_assoc($cliente)) {
        $data['id'] = $row['idcliente'];
        $data['label'] = $row['nombre'];
        $data['contrato'] = $row['contrato'];
        $data['planta'] = $row['planta'];
    ...
    $datos = array();
    $nombre = $_GET['pro'];
    $producto = mysqli_query($conexion, "SELECT * FROM producto WHERE codigo LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%' AND estado = 1 AND existencia >= 1");
    while ($row = mysqli_fetch_assoc($producto)) {
        ...
        $data['name_archivo'] = $row['name_archivo'];
        array_push($datos, $data);
    }
    echo json_encode($datos);
    die();
    ...
    if ($insertar) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $insertarDet = mysqli_query($conexion, "INSERT INTO detalle_venta(id_producto, id_venta, cantidad, precio, id_cliente) VALUES ($id_producto, $ultimoId, $cantidad, '$precio', '$id_cliente')");
